...
function AudioReview() {
  ...
  const fetchAudioList = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/audio/list');
  ...
  const handleDelete = async (filename) => {
    try {
      const res = await axios.delete(`http://localhost:5000/api/audio/delete/${filename}`);
      console.log(res.data.message);
      await fetchAudioList();
    } catch (err) {
      console.error('Error deleting audio:', err.response?.data || err.message);
      setError(`Failed to delete ${filename}: ${err.response?.data.message || err.message}`);
    }
  };
  ...
  const handleRename = async (oldFilename) => {
    ...
    try {
      const res = await axios.put(
        `http://localhost:5000/api/audio/rename/${oldFilename}`,
        { newFilename },
      ...
      setError(`Failed to rename ${oldFilename}: ${err.response?.data.message || err.message}`);
    }
  };
      ...
      <ul>
        {audioList.map((audio) => (
          <li key={audio._id} style={{ marginBottom: '10px' }}>
            ...
            <audio
              controls
              src={`http://localhost:5000/api/audio/stream/${audio.filename}`}
              style={{ marginLeft: '10px' }}
            />
            <button onClick={() => handleDelete(audio.filename)} style={{ marginLeft: '10px' }}>
              Delete
            </button>
            <div style={{ marginTop: '5px' }}>
              <input
                ...
                onChange={(e) => handleRenameChange(audio.filename, e.target.value)}
